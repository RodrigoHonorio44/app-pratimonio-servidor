...
import { AlertOctagon, Image } from "lucide-react";

const ImpressaoLaudoTecnico = ({
  equipamento,
  diagnosticoTecnico,
  ...
  const temHistorico = Array.isArray(historicoManutencoes) && historicoManutencoes.length > 0;

  return (
    <div
      id="secao-laudo-oficial"
      className="bg-white w-full max-w-[850px] font-sans text-slate-900 mx-auto p-2"
    >
      <div className="w-full flex flex-col justify-between h-full corpo-documento-print">
        <div>
          {/* CABEÇALHO COM LOGOS */}
          <div className="flex items-center justify-between gap-2 mb-2 pb-2 border-b border-slate-200 w-full cabecalho-logos">
            <img
              src="/Imagem1.png"
              alt="Logo 1"
              className="h-8 sm:h-9 w-auto max-w-[22%] object-contain"
            />
            <img
              src="/Imagem2.png"
              alt="Logo 2"
          ...
          <div className="text-center space-y-0.5 border-b-2 border-slate-800 pb-2 mb-3">
            <h2 className="text-sm sm:text-base font-black uppercase tracking-wide">
              Laudo Técnico de Inviabilidade e Substituição de Bem
            </h2>
          ...
          <div className="grid grid-cols-2 gap-y-1 text-[11px] mb-2.5 border border-slate-200 p-2 rounded-xl bg-slate-50/50">
            <div className="capitalize">
              <strong>Equipamento / Ativo:</strong> {equipamento.nome}
            </div>
            <div>
              <strong>Nº de Patrimônio (TAG):</strong>{" "}
              <span className="font-mono font-bold uppercase">
                #{equipamento.patrimonio || "S/P"}
              </span>
            </div>
            <div className="capitalize">
              <strong>Unidade de Origem:</strong> {equipamento.unidade}
            </div>
            <div className="capitalize">
              <strong>Setor de Alocação:</strong> {equipamento.setor}
            </div>
            <div>
          ...
          {/* CORPO DO LAUDO */}
          <div className="space-y-2.5 text-xs leading-normal text-justify">
            <div>
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700 uppercase">
                      {historicoManutencoes.map((os) => (
                        <tr key={os.id} className="hover:bg-slate-50/50">
                          ...
                          <td className="p-1 font-medium break-words">
                            {os.descricaoDefeito || os.descricaoProblema}
                          </td>
                          <td className="p-1 text-slate-600 bg-slate-50/30 break-words">
                            {os.solucaoTecnica || "intervenção técnica s/ r."}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {/* REGISTRO FOTOGRÁFICO DO ATIVO */}
            {fotoUrl && (
              <div>
                <h4 className="font-bold text-[10px] uppercase text-slate-500 tracking-wider mb-1 flex items-center gap-1">
                  <Image size={12} /> {temHistorico ? "4." : "3."} Registro Fotográfico do Ativo
                </h4>
                <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-50/50 p-2 flex justify-center max-h-[280px]">
                  <img
                    src={fotoUrl}
            ...
            <p className="pt-1 text-[10px] sm:text-[11px] text-slate-700">
              Conclui-se que o referido patrimônio apresenta desgaste oneroso ou
              obsolescência técnica que inviabiliza economicamente qualquer
              intervenção de manutenção corretiva conforme histórico analítico
              anexo. Fica recomendada pelo{" "}
              <strong className="font-bold">Setor de Patrimônio</strong> a baixa
              do registro patrimonial vigente da unidade.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 pt-2">
          <div className="grid grid-cols-2 gap-8 sm:gap-12 text-center text-xs">
            <div className="space-y-1">
              ...
              <p className="font-bold text-slate-700 text-[10px] sm:text-[11px]">
                Técnico Responsável pela Avaliação
              </p>
              <p className="text-[8px] sm:text-[9px] text-slate-400 uppercase">
                Visto Técnico
              </p>
            </div>
            <div className="space-y-1">
