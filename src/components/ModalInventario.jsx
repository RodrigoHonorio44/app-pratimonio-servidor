...
const ModalInventario = ({ equipamento, isOpen, onClose, onAtualizar }) => {
  ...
  // Executa a baixa definitiva integrando com a API
  const executarBaixaDefinitiva = async () => {
    ...
    // Busca o ID do Ativo de forma segura (evita pegar ID de OS)
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto print:p-0 print:bg-white print:static">
      
      {/* ETAPA 1: CONFIRMAÇÃO INICIAL */}
      {etapa === "confirmacao" && (
        <div className="bg-white rounded-[24px] p-6 max-w-sm w-full shadow-2xl text-center space-y-4 border border-slate-100 animate-in fade-in zoom-in-95 duration-150 my-auto">
      ...
      {etapa === "formulario" && (
        <div className="bg-white rounded-[32px] p-6 max-w-md w-full shadow-2xl space-y-5 border border-slate-100 animate-in fade-in zoom-in-95 duration-150 my-auto">
      ...
      {/* ETAPA 3: LAUDO OFICIAL EM FORMATO A4 COM ROLAGEM CORRIGIDA */}
      {etapa === "preview" && (
        <div className="bg-white w-full max-w-[850px] max-h-[92vh] overflow-y-auto shadow-2xl p-6 sm:p-10 flex flex-col justify-between font-serif text-slate-900 mx-auto rounded-[24px] print:rounded-none print:shadow-none print:p-0 print:max-h-none print:overflow-visible animate-in fade-in duration-200 my-auto custom-scrollbar">
          
          {/* Menu Superior de Ações (Fixado no Topo Durante a Rolagem) */}
          <div className="sticky top-0 z-20 bg-slate-100/95 backdrop-blur-md border border-slate-200 p-3 sm:p-4 rounded-2xl mb-6 flex flex-wrap justify-between items-center gap-3 print:hidden shadow-sm">
          ...
          {/* Conteúdo Físico Impresso do Laudo/Termo */}
          <div className="print:p-6">
            ...
            <div className="text-center space-y-1.5 border-b-2 border-slate-800 pb-4 mb-6 font-sans">
              <h2 className="text-lg sm:text-xl font-black uppercase tracking-wide">Termo de Baixa Definitiva de Bem Patrimonial</h2>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Setor de Patrimônio e Inventário Central</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 sm:gap-y-4 text-xs sm:text-sm mb-6 font-sans border border-slate-200 p-4 rounded-xl bg-slate-50/50">
            ...
            <div className="text-xs sm:text-sm leading-relaxed text-justify mb-8 space-y-3">
              <p>
            ...
            <div className="mt-12 sm:mt-16 pt-8 font-sans">
