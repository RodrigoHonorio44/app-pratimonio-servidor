...
export default function ConsultaPatrimonio() {
  ...
  const resultadoConsulta = useMemo(() => {
    ...
    ativos.forEach((item) => {
      ...
      const matchTermo = !termoBuscaNorm || 
        ...
        setorNorm.includes(termoBuscaNorm);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col justify-between bg-white">
        <Header />
        <div className="flex flex-col items-center justify-center my-auto gap-4 py-12">
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-[#F8FAFC] font-sans antialiased text-slate-900">
      <Header />
                    ...
                    {linhasPaginadas.map(([nomeItem, container]) => {
                      ...
                      return (
                        <div key={nomeItem} className="border border-slate-200/70 rounded-2xl overflow-hidden bg-white shadow-3xs transition-all">
                          ...
                          {/* CONTEÚDO EXPANDIDO (TABELA DETALHADA DOS PATRIMÔNIOS COM COLUNA DE SETOR) */}
                          {aberto && (
                                    ...
                                    {container.itens.map((item) => {
                                      ...
                                      return (
                                        <tr key={item.id} className="border-b border-slate-100 last:border-0 hover:bg-white transition-all">
                                          ...
                                          <td className="py-2.5 pr-4 text-slate-700 font-extrabold capitalize whitespace-nowrap">
                                            {item.setor || "—"}
                                          </td>
