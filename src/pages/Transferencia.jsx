...
const Transferencia = () => {
  ...
  // Garante a ordenação alfabética dos setores para a exibição no dropdown
  const setoresOrdenados = (obterSetoresFiltrados() || []).slice().sort((a, b) =>
      ...
      <main className="flex-1 p-4 md:p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            ...
            {/* 2. FILTRO: SETOR ATUAL (CUSTOMIZADO E ORDENADO ALFABETICAMENTE) */}
              ...
              {mostrarDropdown && (
                <div className="absolute left-0 right-0 top-[62px] bg-white border border-slate-200 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto animate-fadeIn">
                  {setoresOrdenados.length > 0 ? (
                    setoresOrdenados.map((setor, index) => (
                      <button
                        key={index}
                        type="button"
                        onClick={() => {
